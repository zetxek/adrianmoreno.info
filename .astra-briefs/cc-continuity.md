...
A model that rewrote whole files produced: zones.js 2014 -> 1326 lines (-688),
55 -> 37 function declarations (-18), ten builders deleted (t1AtlanticPacket,
t2MovingRoom, landmarkTower, canalHouses, houseSpecs, lampPosts, archBridge, sailMasts,
moored, horreoRow), and layouts/partials/race/enhancements.html lost the athlete markup
entirely (23 -> 0 references to race-athlete).
...
## Verify before you report -- this is what the owner checks

 - `hugo --gc --minify` must be clean.
 - `node --test tests/unit/` must pass (currently 71 pass / 0 fail).
 - E2E: start a CI-exact server with livereload enabled and run
   `npx playwright test --workers=1`. Confirm the server is CI-exact by checking that
   `curl -s localhost:1313 | grep -c livereload` is >= 1; a stale production server makes
   the numbers meaningless. Kill the server by port (lsof -ti:1313), never with
   `pkill -f 'hugo server'` (it matches its own command line).
 - Then report the MEASURED numbers: line count and declaration count of zones.js before
   and after, per-zone triangle tallies, unit pass/fail, E2E pass/fail.

## Reporting

State plainly what you changed, what you could not do, and any place the spec was
ambiguous. Do not claim success you did not measure. If you cannot satisfy the guard,
say so and leave the tree in a working state rather than shipping a regression.

Commit the work with the trailer:
